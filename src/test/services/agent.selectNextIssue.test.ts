...
describe('AgentService.selectNextIssue', () => {
  ...
  it('bug in BRAIN_DUMP overrides non-bug in AI_SPEC', () => {
    agent.setBoardState([
      ...
      { id: 2, title: 'Bug', status: 'BRAIN_DUMP', labels: ['bug'] },
    ]);
    const result = agent.selectNextIssue();
  ...
  it('bug in AI_CODE overrides AI_SPEC items', () => {
    agent.setBoardState([
      { id: 1, title: 'Normal', status: 'AI_SPEC', labels: [] },
      { id: 3, title: 'Bug', status: 'AI_CODE', labels: ['type/bug'] },
    ]);
    const result = agent.selectNextIssue();
    expect(result!.issueId).toBe(3);
  });
  ...
  it('bug in higher-priority column wins', () => {
    agent.setBoardState([
      { id: 1, title: 'Bug AI_SPEC', status: 'AI_SPEC', labels: ['bug'] },
      { id: 2, title: 'Bug BRAIN', status: 'BRAIN_DUMP', labels: ['bug'] },
    ]);
    const result = agent.selectNextIssue();
    expect(result!.issueId).toBe(1);
  });
  ...
  it('returns bug even when busy', async () => {
    agent.setBoardState([{ id: 1, title: 'WIP', status: 'AI_SPEC', labels: [] }]);
    agent.setCallback(vi.fn().mockResolvedValue(undefined));
    await agent.startAgent();
    agent.setBoardState([
      { id: 1, title: 'WIP', status: 'AI_SPEC', labels: [] },
